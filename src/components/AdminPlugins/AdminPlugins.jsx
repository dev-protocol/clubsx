...
class AdminPlugins extends React.Component {
  ...
  render() {
    const toggleEnableClasses = "h-[56px] rounded-[999px] bg-[#040B10] p-[16px] cursor-pointer"
    const toggleDisableClasses = "h-[56px] rounded-[999px] b-0 p-[16px] cursor-pointer"

    console.log('This', this.state.plugins);

    return (
      <div className="relative w-[51.05%] left-[5.555%] flex flex-col items-start gap-[64px] p-0">
        ...
        {this.state.plugins.map((plugin, i) => (
          <div className="flex w-full flex-row items-center justify-between gap-[10px] p-0">
            ...
            <div className="border-[##040B10] flex h-[68px] flex-row items-center justify-start gap-[16px] rounded-[99px] border-[3px] border-[#040B10] p-1.5">
              <div
                onClick={!plugin.enable ? () => {} : () => this.toggleActivation(i)}
                className={!plugin.enable ? toggleEnableClasses : toggleDisableClasses}
              >
                <button
