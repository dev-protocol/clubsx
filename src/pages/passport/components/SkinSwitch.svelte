...
<div class={`relative ${passportClass('passport-switcher-container')}`}>
  <span class={passportClass('passport-switcher-content')}>
    <button
      class={`flex items-center justify-center gap-2 hs-button is-outlined !border border! w-fit ${passportClass('passport-switcher')}`}
    ...
    >
      ...
      {/if}
      <p
        class={`font-bold line-clamp-1 ${passportClass('passport-switcher-label')}`}
      >
        {selectedSkinId
          ? (skins?.find((item) => item.id === selectedSkinId)?.name ??
            'Default')
