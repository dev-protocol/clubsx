...
export async function callSimpleCollections(
    provider: BaseProvider, functionName: string, args: any[]
    ):Promise<any> {
    const chainId = await provider.getNetwork().then((network) => {
        return network.chainId;
    });

    const simpleCollectionaddress = address.find((a) => a.chainId === chainId)?.address || defaultAddress.address;
    const contract = new ethers.Contract(simpleCollectionaddress, simpleCollectionsAbi, provider);

    const result: TransactionReceipt = await contract.functions[functionName](...args);
    return result;
}